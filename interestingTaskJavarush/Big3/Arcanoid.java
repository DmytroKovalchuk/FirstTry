...
public class Arcanoid
{
    ...
    {
        drawBoders(canvas);
        for(Brick each : getBricks()){each.draw(canvas);}
        ball.draw(canvas);
        stand.draw(canvas);
    ...
    public void checkBricksBump()
    {
       for(Brick each : bricks){
           if(each.isIntersec(ball)){
               double angel = Math.random() * 360;
               ball.setDirection(angel);
               bricks.remove(each);
               break;
           }
       } //Тут проверь - столкнулся ли шарик с кирпиче     м.
    ...
    {
        if(ball.getY() >= getHeight()){isGameOver = true;}//Если шарик улетел за нижнюю границы - игра окончена.
    }
